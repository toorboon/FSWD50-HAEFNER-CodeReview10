...
if(isset($_POST['signup-submit'])){
	...
	if (empty($username) || empty($email) || empty($password) || empty($passwordRepeat)) {
		...
		$sql = "SELECT uidUsers FROM library_users WHERE uidUsers=?"; //placeholder for executing safe queries
		...
		if (!mysqli_stmt_prepare($stmt, $sql)) {
			...
			exit();
				...
				$sql = "INSERT INTO library_users (uidUsers, emailUsers, pwdUsers,category) VALUES (?,?,?,?)"; //Placeholders put for safer inserts
				$stmt = mysqli_stmt_init($conn);
				if (!mysqli_stmt_prepare($stmt, $sql)) {
					header("Location: ../index.php?error=sqlerror");
					exit();
